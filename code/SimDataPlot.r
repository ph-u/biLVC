...
# in: for i in `ls ../data/*-log.csv | cut -f 1 -d "-" | rev | cut -f 1 -d "/" | rev`;do Rscript SimDataPlot.r ${i};done
# out: result/*-{best}.pdf
# arg: 1
# date: 20220109
...
nAm = argv[1]
#nAm = "2_PAO1_SA25923"
p = read.csv(paste0("../data/",nAm,"-sam.csv"), header=T, stringsAsFactors=F)
t0 = read.csv(paste0("../data/",nAm,"-log.csv"), header=T)
...
##### plot time-series #####
pdf(paste0("../result/",nAm,"-ts.pdf"), width=14)
